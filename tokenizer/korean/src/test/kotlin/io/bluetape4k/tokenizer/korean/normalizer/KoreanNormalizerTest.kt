...
class KoreanNormalizerTest: TestBase() {
    ...
    @Test
    fun `should normalize ㅋㅋ ㅎㅎ ㅠㅠ ㅜㅜ chunks`() = runTest {
    ...
    @Test
    fun `should have correctTypo integrated`() = runTest {
        normalize("가쟝 용기있는 사람이 머굼 되는거즤") shouldBeEqualTo "가장 용기있는 사람이 먹음 되는거지"
    }

    @Test
    fun `should have normalizeCodaN integrated`() = runTest {
    ...
    @Test
    fun `should normalize coda N nouns`() = runTest {
    ...
    @Test
    fun `should not normalize if the input is known in the dictionary`() = runTest {
        normalizeCodaN("누군가") shouldBeEqualTo "누군가"
        normalizeCodaN("군가") shouldBeEqualTo "군가"
    }

    @Test
    fun `should not normalize if the input is an adjective or a verb`() = runTest {
    ...
    @Test
    fun `should correct typos`() = runTest {
