...
suspend fun <T, U, R> withRateLimiter(
    ...
    rateLimiter.decorateSuspendBiFunction(bifunc).invoke(param1, param2)
...
fun <T, U, R> RateLimiter.decorateSuspendBiFunction(bifunc: suspend (T, U) -> R): suspend (T, U) -> R = { t: T, u: U ->
 ...
 * @return RateLimiter로 decorate된 suspended function
 */
suspend fun <T, U, R> RateLimiter.executeSuspendBiFunction(t: T, u: U, bifunc: suspend (T, U) -> R): R {
    awaitPermission()
    return bifunc(t, u)
}
