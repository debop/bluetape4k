    ...
    inner class CoDecoratorsForRunnableTest {
        ...
        fun `성공하는 runnable에 대한 retry decoration 하기`() = runSuspendTest {
            ...
            val result = runCatching {
                io.bluetape4k.infra.resilience4j.CoDecorators.ofRunnable { service.run() }
                    .withRetry(retry)
                    .invoke()
            }
        ...
        fun `성공하는 runnable에 대해 retry 와 circuit breaker를 decoration 하기`() = runSuspendTest {
            ...
            val decorated = io.bluetape4k.infra.resilience4j.CoDecorators.ofRunnable { service.run() }
        ...
        fun `예외를 발생하는 runnable에 대한 retry decoration하기`() = runSuspendTest {
            ...
            val decorated = io.bluetape4k.infra.resilience4j.CoDecorators
...
//            jcache.clear()
//            val coroutinesCache = CoroutinesCache.of(jcache)
//
//            val decorated = CoDecorators
//                .ofFunction1 { s: String -> service.execute(s) }
...
//
//    @Nested
//    inner class CoDecoratorsForFunction2Test {
//
//        val input1 = 21
//        val input2 = 23
//        val output = 44
