 ...
 */
fun <K, V> GroupedFlow<K, V>.toValues(): Flow<List<V>> = flow {
    emit(this@toValues.toList())
}
 ...
 */
fun <K, V> GroupedFlow<K, V>.toGroupItem(): Flow<GroupItem<K, V>> = flow {
    val values = this@toGroupItem.toList()
    emit(GroupItem(key, values))
}
