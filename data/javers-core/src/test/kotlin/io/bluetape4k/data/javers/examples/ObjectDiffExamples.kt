package io.bluetape4k.data.javers.examples

import io.bluetape4k.data.javers.diff.getChangesByType
import io.bluetape4k.data.javers.diff.getObjectsByChangeType
import io.bluetape4k.data.javers.diff.isListChange
import io.bluetape4k.data.javers.diff.isNewObject
import io.bluetape4k.data.javers.diff.isObjectRemoved
...
class ObjectDiffExamples {
    ...
    fun `엔티티의 컬레션에 요소를 추가하는 변화를 감지합니다`() {
        ...
        diff.getObjectsByChangeType<NewObject>() shouldContainSame listOf(
    ...
    fun `엔티티의 컬레션에 요소를 삭제하는 변화를 감지합니다`() {
        ...
        val removed = diff.getChangesByType<ObjectRemoved>()
        ...
        val removedObjects = diff.getObjectsByChangeType<ObjectRemoved>()
    ...
    fun `속성이 변경된 것을 감지합니다`() {
        ...
        val valueChanges = diff.getChangesByType<ValueChange>()
    ...
    fun `부모 엔티티가 변경된 것을 감지합니다`() {
        ...
        val changes = diff.getChangesByType<ReferenceChange>()
    ...
    fun `@TypeName이 적용되지 않은 일반 클래스도 변화를 감지한다`() {
        ...
        val changes = diff.getChangesByType<ValueChange>()
        with(changes[0]) {
    ...
    fun `최상위 컬렉션을 비교합니다`() {
        ...
        val change = diff.getChangesByType<ListChange>().first()
        log.debug { change }
