...
class JavaStreamExtensionsTest {
    ...
    fun `Sequence as ParallelStream`() {
        ...
        count.get() shouldBeEqualTo 1000
    }
    ...
    @Test
    fun `FastList to Stream`() {
        fastListOf("a", "b", "c").asStream().count() shouldBeEqualTo 3
        fastListOf("a", "b", "c").asStream().max { o1, o2 -> o1.compareTo(o2) }.get() shouldBeEqualTo "c"
    }
    ...
    fun `IntStream to Iterable and List`() {
        ...
        IntStream.range(0, 100).asIterable().toFastList().size shouldBeEqualTo 100
    }

    @Test
    fun `IntStream to IntArray`() {
        IntStream.range(0, 100).toIntArray().size shouldBeEqualTo 100
        IntStream.range(0, 100).toIntArrayList().size() shouldBeEqualTo 100
    }
