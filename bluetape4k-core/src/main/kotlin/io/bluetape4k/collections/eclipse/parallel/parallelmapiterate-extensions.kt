...
fun <K, V> Map<K, V>.parForEach(block: (key: K, value: V) -> Unit) {
    ParallelMapIterate.forEachKeyValue(this) { key: K, value: V -> block(key, value) }
}
...
fun <K, V, R> Map<K, V>.parMap(
    ...
    asIterable().parMap(batchSize, executor, reorder) { mapper(it.key, it.value) }
...
fun <K, V, R> Map<K, V>.parFlatMap(
    ...
    asIterable().parFlatMap(batchSize, executor, reorder) { flatMapper(it.key, it.value) }
