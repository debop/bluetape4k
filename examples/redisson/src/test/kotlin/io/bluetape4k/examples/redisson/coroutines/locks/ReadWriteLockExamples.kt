...
class ReadWriteLockExamples: AbstractRedissonCoroutineTest() {
    ...
    fun `하나의 WriteLock을 잡고, 여러개의 ReadLock을 잡을 수 있습니다`() = runSuspendWithIO {
        ...
        // Write Lock 획득
        log.debug { "WriteLock을 획득합니다... threadId=${Thread.currentThread().id}" }
        ...
        scope.launch(exceptionHandler) {
            log.debug { "WriteLock이 걸린 상태에서 ReadLock 을 획득 시도합니다... -> 실패해야 합니다. ${Thread.currentThread().id}" }
        ...
        log.debug { "WriteLock을 반납합니다... threadId=${Thread.currentThread().id}" }
        ...
        val jobs = List(3) {
            scope.launch(exceptionHandler) {
                ...
                val readLockId = redisson.getLockId(lockName)
                log.debug { "ReadLock을 획득합니다... $it:${Thread.currentThread().id}, readLockId=$readLockId" }
                val readLock = lock.readLock()
                ...
                try {
                    ...
                    log.debug { "ReadLock을 반납합니다... $it:${Thread.currentThread().id}, readLockId=$readLockId" }
