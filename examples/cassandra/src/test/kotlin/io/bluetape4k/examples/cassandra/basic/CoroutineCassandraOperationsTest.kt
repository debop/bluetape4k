...
class CoroutineCassandraOperationsTest(
    ...
    fun `insert and update`() = runSuspendWithIO {
        val user = BasicUser(42L, "debop", "Debop", "Bae")
        operations.insertSuspending(user)

        user.firstname = ""
        operations.updateSuspending(user)

        val loaded = operations.selectOneByIdSuspending<BasicUser>(42L)!!
        loaded shouldBeEqualTo user.copy(firstname = "")
    }

    @Test
    fun `insert in coroutines`() = runSuspendWithIO {
        val users = List(100) {
            BasicUser(it.toLong(), "uname-$it", "firstname-$it", "lastname-$it")
        }
