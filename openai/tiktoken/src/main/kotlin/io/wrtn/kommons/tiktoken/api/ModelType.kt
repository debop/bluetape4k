 ...
 * Model type
 *
 * @property modelName          the code of the model type as used by the OpenAI API
 * @property encodingType  encoding type that is used by this model type
 * @property maxContextLength maximum context length that is supported by this model type
 */
enum class ModelType(val modelName: String, val encodingType: EncodingType, val maxContextLength: Int) {
    ...
    companion object {
        private val modelNameToModelType: Map<String, ModelType> = entries.associateBy { it.modelName }

        @JvmStatic
        fun forModelName(modelName: String): ModelType? {
            return modelNameToModelType[modelName.lowercase()]
        }
    }
}
