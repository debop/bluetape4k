...
import io.bluetape4k.io.http.hc5.http.classicRequest
import io.bluetape4k.io.http.hc5.http.connectionConfig
import io.bluetape4k.io.http.hc5.http.defaultTlsConfig
import io.bluetape4k.io.http.hc5.http.poolingHttpClientConnectionManager
...
class ClientConnectionConfig: AbstractHc5Test() {
    ...
    fun `use connection configuration on a per-route or a per-host`() {
        ...
        httpclient.use {
            URIScheme.values().forEach { uriScheme ->
                val request = classicRequest(Method.GET) {
                    setHttpHost(HttpHost(uriScheme.id, "httpbin.org"))
                    setPath("/headers")
                }
