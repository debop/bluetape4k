...
class BenchmarkExtensionTest {
    ...
    fun `기본 시각 단위로 Benchmark 결과를 출력`() {
        ...
        publishedEvents.size shouldBeEqualTo 1
        toReportEntryKey(publishedEvents[0]) shouldBeEqualTo "Elapsed time[%s] for `canBenchmark`()".format(TimeUnit.MILLISECONDS.name)
    }
    ...
    fun `사용자 정의 시각 단위로 Benchmark 결과를 출력`() {
        ...
        publishedEvents.size shouldBeEqualTo 1
        toReportEntryKey(publishedEvents[0]) shouldBeEqualTo "Elapsed time[%s] for `canBenchmark`()".format(TimeUnit.MICROSECONDS.name)
    }
    ...
    private fun getReportEntries(): List<ExecutionEvent> {
        return listener.getEventsByType(ExecutionEvent.EventType.REPORTING_ENTRY_PUBLISHED)
    }
